...
test.describe('Search and Filter', () => {
    ...
    test('can filter initiatives by name in timeline and data manager', async ({ page }) => {
        ...
        // "Enterprise CIAM" and "Web Channel Integration" should be visible
        await expect(page.getByText('Enterprise CIAM', { exact: true })).toBeVisible();
        await expect(page.getByText('Web Channel Integration', { exact: true })).toBeVisible();

        // Type "Enterprise" into the new search input in the header
        ...
        // "Enterprise CIAM" should remain visible
        await expect(page.getByText('Enterprise CIAM', { exact: true })).toBeVisible();
        // "Web Channel Integration" should be hidden
        await expect(page.getByText('Web Channel Integration', { exact: true })).toBeHidden();
        ...
        await expect(rows.first().locator('input[type="text"]').first()).toHaveValue('Enterprise CIAM');

        // Clear search
        await searchInput.clear();

        // Both should be back in Data Manager (Total of 6 default initiatives)
