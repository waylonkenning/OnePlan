...
test.describe('Applications — InitiativePanel dropdown', () => {
  ...
  test('Application assignment saves and persists after reload', async ({ page }) => {
    test.skip(true, 'Flaky - initiative panel does not open reliably after reload');
    await page.waitForSelector('[data-testid="asset-row-content"]', { timeout: 5000 });
    const bar = page.locator('[data-initiative-id="i-ciam-passkey"]').first();
    await bar.click();
    ...
    const appDropdown = panel.locator('[data-testid="initiative-application"]');
    await expect(appDropdown).toBeVisible({ timeout: 5000 });
    const options = await appDropdown.locator('option:not([value=""])').all();
    if (options.length === 0) {
      test.skip(true, 'No application options available');
      return;
    }
    const appValue = await options[0].getAttribute('value');
    if (!appValue) {
      test.skip(true, 'Could not get application value');
      return;
    }
    ...
    await bar2.click();
    await page.waitForSelector('[data-testid="initiative-action-edit"]', { timeout: 5000 });
    await page.getByTestId('initiative-action-edit').click({ force: true });
