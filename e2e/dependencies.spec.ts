...
test.describe('Dependency Constraint Validation', () => {
  test('self-referencing dependency does not crash the app', async ({ page }) => {
    const errors: string[] = [];
  ...
  test('dependency referencing a deleted initiative does not crash the app', async ({ page }) => {
    const errors: string[] = [];
    page.on('pageerror', err => errors.push(err.message));
  ...
  test('dragging from an initiative to itself creates no new dependency', async ({ page }) => {
    await waitForApp(page);
...
test.describe('Milestone Dependencies', () => {
  ...
  test('dragging from milestone handle to an initiative creates a dependency', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="milestone-dep-source"]')).toHaveCount(
      await page.locator('[data-testid="milestone-dep-source"]').count()
    );
  });
...
test.describe('Segment Dependencies', () => {
  ...
  test('selected segment shows a dependency-draw handle', async ({ page }) => {
    const bar = await selectFirstSegment(page);
    await expect(bar.locator('[data-testid="segment-action-link"]')).toBeVisible();
  });

  test('dragging handle to initiative creates and persists a dependency', async ({ page }) => {
    const initialDepCount = await page.locator('[data-dep-id]').count();
  ...
  test('dropping on another segment does not create a dependency', async ({ page }) => {
    const initialDepCount = await page.locator('[data-dep-id]').count();
    const segBars = page.locator('[data-testid^="segment-bar-"]');
    test.skip(await segBars.count() < 2, 'Need at least two segment bars for this test');
  ...
  test('clicking segment dependency arrow opens DependencyPanel with segment as source', async ({ page }) => {
    const initialDepCount = await page.locator('[data-dep-id]').count();
...
test.describe('Dependency Visual Enhancements', () => {
  ...
  test('intra-asset initiatives have a sufficient vertical gap', async ({ page }) => {
    ...
    expect(bottomBar.y - (topBar.y + topBar.height)).toBeGreaterThan(30);
  });
...
test.describe('Dependency relationship display', () => {
  ...
  test('arrow colours are correct per dependency type (blocks=red, requires=blue, related=dark)', async ({ page }) => {
    // blocks → red
    const blocksGroup = page.locator('g.cursor-pointer.group').filter({ hasText: 'blocks' }).first();
    await expect(blocksGroup).toBeVisible();
    await expect(blocksGroup.locator('path[stroke]').nth(1)).toHaveAttribute('stroke', '#ef4444');

    // Change to requires → blue
    await blocksGroup.click({ force: true });
    const modal = page.locator('[data-testid="dependency-panel"]');
    ...
    // Change to related → dark, no arrowhead
    await requiresGroup.click({ force: true });
...
test.describe('Initiatives & Dependencies report', () => {
  ...
  test('report is grouped by asset with initiatives listed under their asset', async ({ page }) => {
  ...
  test('shows plain-language dependency sentences without technical labels', async ({ page }) => {
...
test.describe('Perspective-aware dependency sentences', () => {
  test.beforeEach(async ({ page }) => {
    await openReportsDependencies(page);
  });

  test('blocked initiative shows "Blocked:" and blocking shows "Blocking:" sentences', async ({ page }) => {
    const report = page.getByTestId('report-dependencies');
  ...
  test('requiring/required-by sentences use correct perspective and no legacy wording', async ({ page }) => {
    const report = page.getByTestId('report-dependencies');
    await expect(report).toContainText(
...
test.describe('Perspective-aware labels in InitiativePanel', () => {
  ...
  test('blocked and blocking initiative panels show correct perspective labels', async ({ page }) => {
    // Blocked initiative
    ...
    // Reload and check blocking initiative
    await page.goto('/');
    await page.waitForSelector('[data-testid="asset-row-content"]', { timeout: 10000 });
    await openInitiativePanelById(page, 'i-apigw-v2');
    const section2 = page.getByTestId('related-initiatives-section');
    await expect(section2).toBeVisible();
    await expect(section2).toContainText('Blocking');
  });
});
