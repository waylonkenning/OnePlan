...
test.describe('Asset Reordering', () => {
  ...
  test('Assets should swap positions when dragged', async ({ page }) => {
    ...
    const ciamHandle = page.locator('.sticky.left-0.w-64').filter({ hasText: 'Customer IAM (CIAM)' }).first();
    const eiamHandle = page.locator('.sticky.left-0.w-64').filter({ hasText: 'Employee IAM' }).first();
